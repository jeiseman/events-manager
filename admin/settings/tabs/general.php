	...
	<div  class="postbox" id="em-opt-google-maps" >
		...
		<div class="inside">
			<div class="em-boxheader">
				<p><?php esc_html_e('Maps API provides you with ways to display maps of your locations and help site visitors find events near their desired locations.','events-manager'); ?></p>
			...
			<table class="form-table">
				<?php
					em_options_select(__('Map Provider', 'events-manager'), 'dbem_map_provider', array('google' => 'Google Maps', 'osm' => 'OpenStreetMaps'), __('Select the map provider you want to use.', 'events-manager'));
					em_options_radio_binary( esc_html__( 'Enable Maps integration?', 'events-manager'), 'dbem_gmap_is_active', esc_html__( 'Check this option to enable Map integration.', 'events-manager'), '', '.em-maps-enabled' );
				?>
				<tbody class="form-table em-maps-enabled">
					<tbody class="em-google-maps-options">
						<?php
					...
					<tbody class="em-google-maps-options em-google-maps-static">
	...
	<div class="inside">
		...
		<script type="text/javascript">
			jQuery(document).ready(function($){
				$('select[name="dbem_map_provider"]').on('change', function(){
					if( $(this).val() == 'google' ){
						$('.em-google-maps-options').show();
					}else{
						$('.em-google-maps-options').hide();
					}
				}).trigger('change');
