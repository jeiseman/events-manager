var em_maps_provider_osm = {
	...
	load: function(){
		if( !this.loaded ){
			if( typeof L !== 'undefined' && !this.loaded ){
				this.init();
			}
		}
	},
	...
	load_locations: function( element ){
		let el = element;
	...
	load_location: function(el){
		...
		if( lat != 0 || lng != 0 ){
			...
			// Fix for map rendering issue when initially hidden
			var mapContainer = jQuery(el);
			if( mapContainer.is(':hidden') ){
				var observer = new MutationObserver(function(mutations) {
					mutations.forEach(function(mutation) {
						if (mutation.type === 'attributes' && mutation.attributeName === 'style' && mapContainer.is(':visible')) {
							this.maps[map_id].invalidateSize();
							observer.disconnect();
						}
					}.bind(this));
				}.bind(this));
				observer.observe(mapContainer[0], { attributes: true });
			} else {
				this.maps[map_id].invalidateSize();
			}
		}
...
jQuery(document).ready( function($){
	if( typeof EM !== 'undefined' && typeof EM.map_provider !== 'undefined' ){
		if( EM.map_provider === 'osm' ){
